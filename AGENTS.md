...
이 리포는 TogetherTrip의 API Gateway 서버다. 에이전트와 Codex는 이 파일을 우선 읽고, `docs/agents/repo-context.md`, `docs/agents/quality-gates.md`, `.codex/setup.md`를 함께 따른다.

## 작업 원칙

- 문서와 산출물은 한국어로 작성한다. 명령어, 코드 식별자, 파일 경로, 라이브러리명은 원문을 유지한다.
- 구현 전에는 라우팅, 인증 전파, CORS, 공통 필터, 서비스 경계 보호를 확인한다.
- gateway는 외부 클라이언트 진입점이며 도메인 로직을 소유하지 않는다.
- header spoofing, 민감정보 로그, rate limit은 보안 검토 대상으로 본다.
